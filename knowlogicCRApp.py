...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.dbFname = r'KsCrDb.db'
        self.crResultsDir = r"."
        self.dbFullFilename = os.path.join(self.crResultsDir, self.dbFname)
        self.dbFullFilename = os.path.normpath(self.dbFullFilename)
        
        self.projFname = r'G4.crp'
        self.pFullFilename = ''
    ...
    def DisplaypFileBrowser(self):
        ...
        pFile = str(QFileDialog.getExistingDirectory(self, "Select Project File Directory"))
        if (not pFile):
            pFile = self.defaultDBLocation

        self.textBrowser_ProjFile.setText(pFile)

        # Set the pFile based on the user's browse choice
        self.pFullFilename = os.path.join(pFile, self.projFname)
        self.pFullFilename = os.path.normpath(self.pFullFilename)
    ...
    def DisplayViolationsData(self):
        ...
        if self.violationsData: 
            ...
            v = self.violationsData[self.horizontalScrollBar.value()]
            self.textBrowser_Filename.setText(v.filename)
            self.textBrowser_Function.setText(v.function)
            self.textBrowser_Description.setText(v.description)
            self.textBrowser_Details.setText(v.details) 
            self.textBrowser_Severity.setText(v.severity)
            self.textBrowser_Id.setText(v.violationId)
            self.textBrowser_DetectedBy.setText(v.detectedBy)
            
            """ Populate the fields in the Analysis groupbox """
            self.textBrowser_PrevReviewer.setText(v.who)
            self.textBrowser_PrevDate.setText(v.reviewDate)
            self.textBrowser_PrevStatus.setText(v.status)
            
            self.plainTextEdit_Analysis.setPlainText(v.analysis)
